...
$credit_note_info .= format_organization_info();

$credit_note_info .= '</div>';

$pdf->writeHTMLCell(($swap == '1' ? ($dimensions['wk']) - ($dimensions['lm'] * 2) : ($dimensions['wk'] / 2) - $dimensions['lm']), '', '', $y, $credit_note_info, 0, 0, false, true, ($swap == '1' ? 'R' : 'J'), true);

// Bill to
$client_details = '<b>' . _l('credit_note_bill_to') . '</b>';
$client_details .= '<div style="color:#424242;">';
    $client_details .= format_customer_info($credit_note, 'credit_note', 'billing');
$client_details .= '</div>';

$pdf->writeHTMLCell(($dimensions['wk'] / 2) - $dimensions['rm'], '', '', ($swap == '1' ? $y : ''), $client_details, 0, 1, false, true, ($swap == '1' ? 'J' : 'R'), true);


$pdf->Cell(0, 0, _l('credit_note_date') . ': ' . _d($credit_note->date), 0, 1, ($swap == '1' ? 'L' : 'R'), 0, '', 0);

if (!empty($credit_note->reference_no)) {
    $pdf->Cell(0, 0, _l('reference_no') . ': ' . $credit_note->reference_no, 0, 1, ($swap == '1' ? 'L' : 'R'), 0, '', 0);
}
...
$tblhtml = '
<table width="98%" bgcolor="#fff" cellspacing="0" cellpadding="8">
    <tr height="30" bgcolor="' . get_option('pdf_table_heading_color') . '" style="color:' . get_option('pdf_table_heading_text_color') . ';">
        <th width="5%;" align="center">#</th>
        <th width="45%;" align="left">' . _l('credit_note_table_item_heading') . '</th>
        <th width="52%" align="right">' . $qty_heading . '</th>
        <th bgcolor="#fff" width="15%" align="right">' . _l('credit_note_table_rate_heading') . '</th>';

if (get_option('show_tax_per_item') == 1) {
    $tblhtml .= '<th bgcolor="#fff" width="15%" align="right">' . _l('credit_note_table_tax_heading') . '</th>';
}

$tblhtml .= '<th bgcolor="#fff" width="15%" align="right">' . _l('credit_note_table_amount_heading') . '</th>
</tr>';
